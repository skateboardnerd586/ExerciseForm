...
def process_video(video_file, squat_down_threshold=130, squat_up_threshold=150):
    ...
    try:
        ...
        while True:
            ...
            # No flip correction needed - video appears correct without flipping
            # if is_cloud_env:
            #     frame = cv2.flip(frame, 1)
            #     if frame_count == 1:
            #         st.write("Debug - Applied flip correction")
            ...
            if keypoints:
                ...
                cv2.putText(frame, f'Squats: {rep_count}', (50, 170), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 0), 2)
                cv2.putText(frame, f'Phase: {phase}', (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 2)
                ...
                for i, rep_data in enumerate(rep_history[-5:]):  # Show last 5 reps
                    y_pos = y_start + 30 + (i * 140)  # Increased spacing to 140 pixels between reps
...
def analyze_with_openai(rep_data, api_key):
    ...
    try:
        ...
        - Average Left Knee Angle: {avg_left_knee:.1f} deg
        - Average Right Knee Angle: {avg_right_knee:.1f} deg
        - Average Left Hip Angle: {avg_left_hip:.1f} deg
        - Average Right Hip Angle: {avg_right_hip:.1f} deg
        ...
        for i, rep in enumerate(rep_data):
            data_summary += f"""
        Rep {rep['rep_number']}:
        - Left Knee: {rep['left_knee_angle']:.1f} deg
        - Right Knee: {rep['right_knee_angle']:.1f} deg
        - Left Hip: {rep['left_hip_angle']:.1f} deg (if available)
        - Right Hip: {rep['right_hip_angle']:.1f} deg (if available)
        ...
        Please use secodn person to address the user.
        """
...
def create_matplotlib_charts(rep_data):
    ...
    fig.suptitle('Squat Analysis Charts', fontsize=16)
    ...
    if df['left_hip_angle'].notna().any():
        axes[0, 1].plot(df['rep_number'], df['left_hip_angle'], 'g-o', label='Left Hip', linewidth=2)
    if df['right_hip_angle'].notna().any():
        axes[0, 1].plot(df['rep_number'], df['right_hip_angle'], 'm-o', label='Right Hip', linewidth=2)
    axes[0, 1].set_title('Hip Angles by Rep')
    ...
    # Duration over time
    axes[1, 0].plot(df['rep_number'], df['duration_seconds'], 'purple', marker='o', linewidth=2)
    axes[1, 0].set_title('Squat Duration by Rep')
    axes[1, 0].set_xlabel('Rep Number')
    axes[1, 0].set_ylabel('Duration (seconds)')
    axes[1, 0].grid(True, alpha=0.3)
    
    # Symmetry analysis
    knee_diff = df['left_knee_angle'] - df['right_knee_angle']
    axes[1, 1].bar(df['rep_number'], knee_diff, color='orange', alpha=0.7)
    axes[1, 1].set_title('Knee Angle Difference (L-R)')
...
def main():
    ...
    with col2:
        ...
        if st.session_state.video_processed and st.session_state.rep_data:
            ...
            st.info("📊 **Note**: All angles shown are measured at the deepest point of each squat rep (lowest knee/hip angles during the movement).")
            df = pd.DataFrame(rep_data)
